 ...
 * Prioritizes storage over hardcoded levels.
 */
export const getCustomLevel = (id: number): Level | null => {
  // 1. Check local storage first for user-edited hints/levels
  try {
    const saved = localStorage.getItem('neuronodes_custom_levels');
    if (saved) {
      const customLevels = JSON.parse(saved);
      const custom = customLevels[id.toString()] || customLevels[id];
      if (custom) return custom;
    }
  } catch (e) {}

  // 2. Fallback to hardcoded manual levels
